...
type AccountData = { user: { name: string; phone?: string; createdAt: string }; orders: { id: string; status: string; total: number; createdAt: string }[] };
...
export default function AccountPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      <section className="mt-8 rounded-2xl border border-[#E5E7EB] bg-white p-5 sm:p-7"><div className="flex items-center gap-2"><Package className="text-[#2563EB]" size={21} /><h2 className="font-extrabold text-[#111827]">سفارش‌های من</h2></div>{data.orders.length === 0 ? <p className="mt-6 text-sm text-[#6B7280]">هنوز سفارشی ثبت نشده است.</p> : <div className="mt-5 grid gap-3">{data.orders.map((order) => <div key={order.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-[#F8FAFC] p-4 text-sm"><span className="font-bold">{order.id}</span><span>{order.status}</span><span>{new Intl.NumberFormat("fa-IR").format(order.total)} تومان</span></div>)}</div>}</section>
